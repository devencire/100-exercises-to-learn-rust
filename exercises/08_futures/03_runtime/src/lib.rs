    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let first_handle = tokio::spawn(reply_with(first, Arc::clone(&reply_arc)));
    let second_handle = tokio::spawn(reply_with(second, reply_arc));
    let (first_result, second_result) = tokio::join!(first_handle, second_handle);
    first_result.unwrap();
    second_result.unwrap();
}

async fn reply_with<T>(listener: TcpListener, reply_arc: Arc<T>)
    where T: Display + Send + Sync + 'static,
{
    loop {
        let reply_arc = Arc::clone(&reply_arc);
        let (mut socket, _) = listener.accept().await.unwrap();
        tokio::spawn(async move {
            let (_, mut writer) = socket.split();
            writer.write_all(format!("{}", reply_arc).as_bytes()).await.unwrap();
        });
    }
}
